...
BOOL CTreeListDlg::OnInitDialog() {
	...
	for(i = 0; i < 3 * 2; i++) {
		...
		if(i == 0)
			m_cTreeList.Expand(hItem2[i], TVE_EXPAND);
...
void CTreeListDlg::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT pDrawItemStruct) {
	...
	if(uId == IDC_COMBO9) {
		...
		if(pDrawItemStruct->itemState & ODS_FOCUS)
			uColor = RGB(108, 148, 255);
		if(pDrawItemStruct->itemState & ODS_DISABLED)
			uColor = GetSysColor(COLOR_3DFACE);

		SetBkColor(pDrawItemStruct->hDC, uColor);
		ExtTextOut(pDrawItemStruct->hDC, 0, 0, ETO_OPAQUE, &pDrawItemStruct->rcItem, NULL, 0, NULL);

		m_cChecks.Draw(pDc, pDrawItemStruct->itemID * 2  , sPt, ILD_TRANSPARENT);
		sPt.x += 24;
...
void CTreeListDlg::OnUpdateAutoEdit() {
	...
	if(m_bDropDown)
		iMode |= TVAE_DROPDOWN;
	if(m_bFullWidth)
		iMode |= TVAE_FULLWIDTH;
	if(m_bIconClick)
		iMode |= TVAE_ICONCLICK;
	if(m_bOnlyReturn)
		iMode |= TVAE_ONLYRETURN;
	if(m_bStateEnable)
		iMode |= TVAE_STATEENABLE;

	if(m_bPtrList) {
		switch(m_iAutoEditMode) {
			case 0:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_NONE);
				break;
			case 1:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_EDIT   | iMode);
				break;
			case 2:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_COMBO  | iMode, pTextList);
				break;
			case 3:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_CBLIST | iMode, pTextList);
				break;
			case 4:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_STEP   | iMode, pTextList, 2);
				break;
			case 5:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_STEPED | iMode, pTextList, 2);
				break;
			case 6:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_CHECK  | iMode);
				break;
			case 7:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_CHECKED | iMode);
				break;
		}
	} else {
		switch(m_iAutoEditMode) {
			...
			case 1:
				...
				break;
			case 2:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_COMBO  | iMode, '|', _T("Text|Auto|Bike|Sun"));
				break;
			case 3:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_CBLIST | iMode, '|', _T("Text|Auto|Bike|Sun"));
				break;
			case 4:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_STEP   | iMode, '|', _T("Yes|No|NoneSense|Dummy"), 2);
				break;
			case 5:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_STEPED | iMode, '|', _T("Yes|No|NoneSense|Dummy"), 2);
				break;
			case 6:
				m_cTreeList.SetColumnAutoEdit(3, TVAE_CHECK  | iMode);
				break;
	...
	switch(m_iAutoEditMode) {
		case 2:
			m_cTreeList.SetColumnAutoIcon(3, 10);
			break;
		case 3:
			m_cTreeList.SetColumnAutoIcon(3, 10);
			break;
		case 4:
			m_cTreeList.SetColumnAutoIcon(3, 15);
			break;
		case 5:
			m_cTreeList.SetColumnAutoIcon(3, 15);
			break;
