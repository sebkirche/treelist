...
while(<DATA>){
    ...
    if(/#\s*define\s+(\w+)\s+([^\/]+).*/){
        my ($cst, $val) = ($1, $2);
        my $pbval = $val;
        $pbval =~ s!(\([A-Z]\w*\))!!g;          #remove CAST opreators
        $pbval =~ s/(0x[\da-fA-F]+)/eval($1)/ge;#resolve hexa numbers
        $pbval =~ s/\b(\d+)U/$1/g;#resolve U-numbers
        my $type = 'ulong';
        $type = 'long' if $val=~/-/;
        $type = 'string' if $val=~/['"]/;
        $pbval =~ s/([+-])/ $1 /g;
        $pbval =~ tr/|/+/;
        #~ print "//" if $pbval =~ /[<>]/;
        #~ say "constant $type $cst = $pbval   // ( $val )";
        #~ $DB::single=1 if $cst eq 'TVM_INSERTITEM';
        if(exists $constants{$cst}){
            #Prefere unicode definition
            next if $constants{$cst}[0]=~/W$/ and $pbval=~/A$/;
        }
        $constants{$cst}=[ $pbval, $val ];
    }
    #~ else{
        #~ say "//$_";
    #~ }
...
#Force TV_UINICODE to true
$constants{'TV_UINICODE'}=[1,1];

foreach my $rx ( @used_regex ){
    $used{$_} = undef for grep { /$rx/ } keys %constants;
}

foreach my $cst( sort keys %used ){
    my ($pbval, $val) = @{$constants{$cst}};
    $pbval = get_value($pbval);
    my $type = 'ulong';
    $type = 'long' if $val=~/-/;
    $type = 'string' if $val=~/['"]/;
    say "constant $type $cst = $pbval";
}

sub get_value{
    my $pbval = shift;
    #~ $pbval =~ s/(\w+)/get_constant($1)/ge;
    while($pbval =~ /([A-Za-z]\w*)/){
        my $word = $1;
        my $value = get_constant($word);
        $pbval =~ s/\b$word\b/$value/g;
    }
    eval($pbval);
}
...
sub get_constant{
    my $cst = shift;
    #~ print "get_constant('$cst') .. ";
    while(exists $constants{$cst}){
        $cst = $constants{$cst}[0];
    }
    #~ print "$cst\n";
    get_value( $cst );
